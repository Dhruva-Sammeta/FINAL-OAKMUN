...
async function lookup(email) {
  ...
  try {
    ...
    if (!data) {
      ...
      return { statusCode: 404, data: { error: 'No delegate found with that email.', _debug: 'cf-diag', _logErr: logErr } };
    }
    const logErr = await logLookup(db, email, true, data.id);
    return { statusCode: 200, data: { delegate: data, _debug: 'cf-diag', _logErr: logErr } };
  } catch (err) {
    console.error('lookup-qr error:', err);
    return { statusCode: 500, data: { error: 'Something went wrong. Please try again.', _debug: 'cf-diag', _catchErr: String(err && err.message) } };
  }
}
